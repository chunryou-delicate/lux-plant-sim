...
⛔⛔ 2026-09-07 — **이 자를 다시 돌리기 «전»에 읽을 것**

  ① `portrait_jachwi_blank` 를 아직 쓴다 ⇒ ★ 그 파일은 **`portrait_jachwi_numb` 로 이름이 바뀌었다**
     (`0d4f1b9` · 「빈 판」으로 읽혀 표정 키도 `numb` 가 됐다).
     ⇒ ⛔ 그대로 돌리면 **`blank` 가 되살아나** 안 쓰는 파일이 하나 늘고
       ⇒ ⇒ 다음 사람이 「왜 둘이지」로 헷갈린다. **아래 목록을 고치고 돌릴 것.**

  ② 2026-09-07 에 «클링»으로 낯 여섯이 늘었다 —
     moni: worry · teach · proud · cheer · calm  ·  jachwi: curious
     ⇒ ✔ 그 여섯은 **이 자의 목록에 «없어» 안 건드린다**(이름이 안 겹치는 것을 확인함).
     ⇒ ★ 그것들은 `tools/char/fit_portrait.py` 가 «바깥 그림»을 맞춰 넣은 것이다.
       규격(600×800 · 투명 · 바닥선)은 그 자가 «재서» 맞춘다.
