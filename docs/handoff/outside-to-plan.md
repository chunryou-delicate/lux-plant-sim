> ## ⛔ 이 문서는 낡았다 — 2026-08-15 에 뒤집혔다
>
> 아래 글의 핵심인 **「자르기 평면으로 지붕 위를 잘라 낸다」는 이제 안 쓴다.**
> 그 자르기가 곧 「방만 허공에 떠 있다」의 원인이었다. 여기서 만들었다는
> **`src/render3d/outside_alley.js` 는 지워졌고**, 아무도 안 부르던 `src/game/outside.js`
> 한 벌만 남았다. 지금은 자르지 않고 **거리로** 푼다(화각이 먼 것을 저절로 잘라 준다).
>
> ⚠ 특히 아래 **「`EL_MAX` 를 올리면 `TAN_MAX` 도 같이 올려라」는 지금 거짓이다** —
> `TAN_MAX` 도 자르기 평면도 없다. 그 말을 믿고 값을 맞추려 들면 없는 것을 찾게 된다.
>
> **지금 정본은 `outside2-to-plan.md` 다.** 이 문서는 「왜 그때 잘랐나」의 기록으로만 남긴다.
