...
### 렌더 오류 경계 — ★ 앞 문단을 정정합니다 (2026-08-22 재확인)

**앞서 "growth 가 삼키고 정상 반환한다 → 렌더 오류로는 어긋남이 생기지 않는다"고 적었는데,
지금은 사실이 아닙니다.** growth 가 렌더 경계를 `advanceTo`·`setGrowth` 까지 올리면서
(`3578452`) **던지는 쪽으로 바뀌었습니다.** 같은 재현을 다시 돌린 결과입니다.

```
buildPlant() 를 일부러 던지게 바꿔 넣고 advanceTo 호출
  throw "[생장] 3D 를 다시 그리지 못했습니다 — 그리기: [화면] 렌더 실패"
  코어 day 4→5 · growth 달력 147→148 · 유효 147→148
  coreRolledBack=false · S.desync={ coreDay:5, growthCalendar:148 }
```

⇒ **위 표의 2행(“하루 갔음 → 되감지 않는다”)이 안전망이 아니라 실제 경로입니다.**
코어는 되감지 않고 날짜를 growth 에 맞춘 뒤 `S.desync` 에 남기고, 화면은 진행을 잠급니다.
동작은 의도대로지만 **문장이 낡아 있었습니다** — 이 저장소가 자주 앓는 병이라 여기 남깁니다.

> 던지는 쪽이 낫다고 봅니다. 삼키면 "그림만 안 그려진 하루"가 조용히 지나가고,
> 플레이어에게는 아무 일도 안 일어난 것처럼 보입니다.
