# 무늬 잎 22갈래 — **생김새만** 적는다 (2026-08-29 · leaf)

## ⛔⛔ 제목을 고쳤다 — **이 22갈래 중 19는 «게임에 붙어 있다»**

처음 이 문서를 「게임에 안 붙은 22갈래」로 냈다. **틀렸다.**
등급표가 짧은 id + `matNum` 으로 적고, `matNum` → 파일 다리가 `plant_grow.html` 에 있어서
내가 못 봤다. 자세한 것은 `leaf_map.md §㉠`.
⇒ ★ **아래 생김새 표는 그대로 쓸모 있다** — 어느 잎이 어떻게 생겼는지는 안 바뀐다.
  ⛔ 다만 「안 붙었다」는 말만 물린다. 정말 안 붙은 것은
  `monstera_leaf_young_albo` · `pothos_mint_simple` · `pothos_variegated` **셋**이다.
