...
## 현재 진행 (2026-07-20 기준)
**몬스테라 생장 생성기**(`plant_grow.html`, 1,316줄) — 순서도 캐논을 런타임으로 옮기고 튜닝 중.
- 생장점당 잎 1개 · 새 마디는 생장점 **아래** 삽입 · 혹은 "잎이 중간잎 되는 순간" 1개씩 랜덤
- 줄기는 **이음새 없는 튜브**로 직접 생성(마디 GLB 이어붙이기 폐기) · 축마다 고정 아치로 자연스러운 곡선
- 잎↔엽초 **일체화**(시드 바뀌어도 안 틀어짐) · 엽초 14종이 잎 단계별로 독립 조정
- 변이(알보) 8종 투입 · 잎마다 20% 독립 발현 · 무늬 텍스처는 틴트 금지
- 성장도 **3년(1095일)** · 전체 성장 게이지 하나로 줄기·엽초·잎이 비율 유지한 채 함께 커짐
- **진행 상세: `몬스테라_생장엔진_진행.md`** (측정 기반 버그 수정 기록)
