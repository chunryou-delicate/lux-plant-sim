...
await clear();
await page.eval(`(()=>{const S=window.__S(); S.tutorial.lamp.unlocked=true; S.tutorial.lamp.owned=2; S.tutorial.lamp.placed=2; S.lamps.count=2;
...
const menu=async(t)=>console.log(t, await page.eval(`(()=>{
  ...
  return JSON.stringify({이름:on('furnName'), 옮기기:on('furnMove'), 돌리기:on('furnTurn'),
    켜기끄기:on('furnLamp'), 물릴자리:on('furnMount'), 까닭:on('furnWhy'), 메뉴:!!document.getElementById('stage').classList.contains('furnpicked')});})()`));
