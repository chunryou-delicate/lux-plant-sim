/* ★★ **이 자는 「채소를 안 늘리는 사람」을 잰다 — 바닥값이다** (2026-08-23 plan)
 *  ------------------------------------------------------------
 *  ⚠⚠ **이름이 `balance` 라 「밸런스가 틀렸다」로 읽힌다. 그게 아니다.**
 *  이 자는 **시루 1개 고정**으로 돌린다. 채소를 퀵스트대로 안 늘리는 판이다.
 *  ⇒ 여기서 이사 0% 가 나와도 **밸런스가 깨졌다는 뜻이 아니다.**
 *    「채소를 안 늘리면 못 나간다」는 뜻이고, 그것은 설계일 수 있다.
 *  ★ **「안내를 따르는 사람」을 재는 자는 `test_banjiha_routes.mjs` 다**(시루를 퀵스트대로 늘린다).
 *    둘이 정반대로 말하는 것처럼 보이면 **두 자가 다른 사람을 재는 것**이다.
 *  ★ 실측(2026-08-23 밤): 여기 못 나간 24판 중 **돈에서 막힌 것이 20판**이고
 *    무늬는 이미 팔았다. 모자란 돈이 이사비 **전액**이라 지갑이 사실상 0 이다.
 *  ⏸ 이름을 바꾸자는 말이 있었지만(총괄 2026-08-23), 바꾸면 `test_` 글눵에서
 *    빠져 **스위트에서 사라진다.** 그건 아침에 정한다 — 지금은 머리말로 막는다.
 *
