/* ============================================================
   ...
   ★ 2026-08-04 개편 — 재는 것이 넷으로 늘었다:
       ① 시루 개수      시루를 늘리고 **어긋나게 돌리면** 절감이 는다는 것을 눈으로 본다
       ② 자리(품질)     어두운 자리 vs 밝은 자리 — 빛 축이 살아 있나
       ③ ★시차         물 주는 간격을 벌리면 겹침이 풀린다 — 짜임새가 실제로 값이 되나
...
import { createFirstPlayState, firstPlayRulesFromBalance, placeBeansprout,
         ...
         waterBeansprout, beansproutReady, makeCropPot,
...
function run({ sirus = 1, dli = 0.05, days = 30, stagger = 1, harvestLag = 0 } = {}) {
  ...
  for (let d = 1; d <= days; d++) {
    /* ── 플레이어의 행위 ①: **물주기 = 회전 시작** (2026-08-04 새 규칙 · §물주기).
       `stagger` 만큼 날을 띄워 한 시루씩 시작한다 — 0 이면 그날 대기를 전부 시작한다.
       d-1 이 "지금 화면에 뜬 날"이다(loop 은 증가 전 S.day 를 본다). */
    const shown = d - 1;
    if (stagger === 0) waterBeansprout(fp, shown, { all: true });
    else if ((d - 1) % stagger === 0) waterBeansprout(fp, shown);

    /* ── [다음 날] ─ 하루가 간다. ★여기서는 안 거둬진다 */
    {
      const ev = advanceBeansproutDay(fp, dli);
      dry += ev.idle;                       // 시작을 기다린 시루-날 수(마른 날이 아니다)
    }
    ...
    if (beansproutReady(fp.beansprout)) {
      ...
      if (d - readySince >= harvestLag) {
        ...
        harvests += ev.harvestedPots; spoiled += ev.spoiledWon || 0; readySince = null;
        /* ── 플레이어의 행위 ③: 다시 심기 (거둔 그 날에 바로). **거둔 시루만** 심는다 */
        const rs = resowBeansprout(fp, { day: d });
        seed += rs.resown * SEED_BUY;
      } else waited++;
    }
  }
...
console.log('① 시루 개수 — 하루씩 어긋나게 시작하면 (30일 · 어두운 자리 DLI 0.05)');
for (const sirus of [1, 2, 3, 4, 5, 6]) {
...
console.log(`\n③ ★시차 — 물 주는 간격이 곧 값이다 (30일 · 시루 ${R.harvestDays}개 · 어두운 자리)`);
for (const [ko, stagger] of [['같은 날 전부', 0], ['하루씩 걸러', 1], ['이틀씩 걸러', 2]]) {
  const r = run({ sirus: R.harvestDays, stagger, days: 30 });
  console.log(`  ${ko} | 30일 절감 ${won(r.total)} · 하루평균 ${won(r.total / r.days)} · ` +
              `수확 ${r.harvests}시루 · 씨앗값 ${won(r.seed)}`);
}
console.log(`  ⤷ ★${R.harvestDays}일 주기니까 ${R.harvestDays}개까지 하루씩 어긋나게 두면 **매일 온전한 값**이 나온다.`);
console.log('    같은 날 몰아 시작하면 겹쳐서 3,000 → 2,000 → 1,000 → 0원으로 깎인다 — 벌이 아니라 짜임새다.');
