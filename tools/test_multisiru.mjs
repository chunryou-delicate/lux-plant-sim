...
const GOLD = {
  ...
  /* ★★ 2026-08-09 — **몬스테라 그루 높이 여덟 줄이 움직였다.** 회귀가 아니라 확정이다.
     ...
     ★★ 2026-08-15 — **또 움직였다. 이번엔 leaf_interval 이 아니다.** 위 ⚠ 가 시킨 대로
       먼저 봤고, 표는 안 바뀌어 있었다. 원인은 **가지의 발아 순서를 캐논에 맞춘 것**(`368b378`):
       가지 축이 나는 날 첫 마디(잎자루)를 같이 심게 했다. 캐논이 *"눈에서 가는 잎자루
       먼저(새순 없이)"* 라고 못 박고 있는데 그 반대로 돌고 있었다.

         유효일   높이 옛 → 새          0.14 화분   옛 → 새
           60     0.32667 → 0.32431     0.22867 → 0.22702
          143     0.54442 → 0.54151     0.38109 → 0.37905
          146     0.54746 → 0.54453     0.38322 → 0.38117
          300     0.69423 → 0.68703     0.48596 → 0.48092
       여덟 줄 **전부** 움직였다. 0.5~1% 작아졌고 화분·지름·자식 수는 안 움직였다.

       ★ 높이가 움직인 까닭은 마디가 늘어서가 아니다 — `addSeg` 가 난수를 세 번 쓰므로
         첫 마디를 앞당기면 **난수가 한 칸 밀린다.** 마디 **수**가 아니라 방향·길이가 밀렸다.

       ⚠⚠ **여기서 재는 자에 한 번 속았다. 다음 사람도 속을 자리라 적어 둔다.**
         이 검사는 어긋난 것을 **`slice(0, 4)` 로 넷만 찍는다**(:296). 그걸 모르고
         「넷만 어긋났다 = 146·300 은 멀쩡하다」로 읽고 그렇게 적었다. 앞 넷을 고치자
         **뒤 넷이 새로 나타났다.** 여덟 줄 다 움직여 있었던 것이다.
         ⇒ **안 나온 것을 「없다」로 읽지 마라.** 목록이 잘려 있는지부터 봐라.

       ★ 회귀가 아닌 것을 이렇게 확인했다: `368b378^` 로 워크트리를 떼어 같은 검사를 돌리니
         **②-C 가 통과**했다. 즉 이 커밋 하나가 원인이고, 잎 나이표·단계 일정 34줄·
         400일 잎 수는 **하루도 안 움직였다**(`monsterabranch-to-plan.md`).
       ⇒ 그래서 값을 **갱신한다.** 지우지 않고 옛 값을 위 표에 남긴다. */
  monstera: {
    ...
    'g60|-':     { children: 1, diameter: 0.20, pot: { w: 0.19811, h: 0.13321, d: 0.19743 }, wholeH: 0.32431 },
    'g60|0.14':  { children: 1, diameter: 0.14, pot: { w: 0.13868, h: 0.09324, d: 0.1382  }, wholeH: 0.22702 },
    'g143|-':    { children: 1, diameter: 0.20, pot: { w: 0.19811, h: 0.13321, d: 0.19743 }, wholeH: 0.54151 },
    'g143|0.14': { children: 1, diameter: 0.14, pot: { w: 0.13868, h: 0.09324, d: 0.1382  }, wholeH: 0.37905 },
    'g146|-':    { children: 1, diameter: 0.20, pot: { w: 0.19811, h: 0.13321, d: 0.19743 }, wholeH: 0.54453 },
    'g146|0.14': { children: 1, diameter: 0.14, pot: { w: 0.13868, h: 0.09324, d: 0.1382  }, wholeH: 0.38117 },
    'g300|-':    { children: 1, diameter: 0.20, pot: { w: 0.19811, h: 0.13321, d: 0.19743 }, wholeH: 0.68703 },
