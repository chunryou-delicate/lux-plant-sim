...
const BEFORE = {
  /* ★★ 2026-08-23 — **협탁 자리 한 칸만 움직였다.** (`furniture_pastel §tierSlots`)
     ──────────────────────────────────────────────────────────────────
     협탁이 `ca3f8f8` 로 2×2 칸이 되면서 상판 **한가운데가 칸 경계**가 됐다. 커서는
     칸 한가운데에만 내려앉으므로(`room_view §snapAxis` — "이제 칸이 정본이고 놓는 자리가
     칸 한가운데") 그 자리는 **어느 칸을 겨눠도 안 붙는 자리**였다 — 가장 가까운 칸까지
     0.1768m, `SLOT_GOVERN_R` 0.04 의 네 배다. 자리를 칸 한가운데로 옮겼다.
     ⇒ 움직인 칸은 **하나뿐이다**: `nightstand:0` 0.38 → **0.44**.
     ★★ **창턱은 글자 하나 안 바뀌었다** — 3.68 / 6.02 / 6.06 그대로. 위 §①-b 가 지키는
       「등 하나로 문턱 6.0 을 여유 0.02 로 넘는다」, 이 방에서 제일 얇은 얼음은 안 밟았다.
     ★ **자리 수는 안 늘렸다** — 반지하 15칸, 여섯 방 325칸 그대로.
       2×2 상판에 네 칸을 주면 더 낫다는 실측(0.44·0.43·0.32·0.32 — 넷 다 닿는다)은
       `docs/engine/slot_cell_align.md` 에 남겼다. 자리를 늘리는 것은 밸런스라 따로 올렸다.
     ★ 값은 손으로 안 적었다: `BYEOT_REGEN=1` 로 다시 뽑았다. */
  /* ★★★ 2026-08-17 — **책상과 협탁이 움직여 세 칸이 바뀌었다.** (박사님:
       *"책상이 2*6 차지하는데 칸수는 2*5네 수정해줘"* · *"협탁 크기가 애매해,
         할 거면 2*2 크기로 해서 주던지 해"*)
     ──────────────────────────────────────────────────────────────────
     · 책상 x 1.3 → **1.375** — 크기는 안 건드렸다. 폭 1.25 는 이미 0.25 격자 다섯 칸인데
       앞 모서리가 격자 선에 안 앉아 **여섯 열을 걸치고 있었다.**
     · 협탁 0.42×0.36 → **0.50×0.50**(2×2 칸) · 자리도 격자 선으로(x 0.5 · z −1.5).
     ⇒ 움직인 칸은 **셋뿐이다**: `desk:0` 0.58→0.56 · `desk:1` 0.18→0.16 ·
       `nightstand:0` 0.29→**0.38**(창 쪽으로 0.175 나오고 커졌다).
     ★★ **창턱은 글자 하나 안 바뀌었다** — 3.68 / 6.02 / 6.06 그대로다.
       위 §①-b 가 지키는 「등 하나로 갈라짐 문턱 6.0 을 여유 0.02 로 넘는다」가
       이 방에서 제일 얇은 얼음인데, 거기는 안 밟았다.
     ★ 값은 손으로 안 적었다: `BYEOT_REGEN=1` 로 다시 뽑았다. */
...
/* ★★★ 합격선이 **또 바뀌었다 — 2026-08-16 G-16**.
   ...
   ⇒ 등을 **창 위 벽**으로 올렸고, 이제 **첫 등이 창턱을 7.07 로 올려 문턱 6.0 을 넘긴다.**
     계약이 뒤집힌 것이 아니라 **자리가 바뀌어서 결과가 뒤집혔다.** 그래서 여기서 잰다:
       ...
       ㉣ 과광 16.0 도 안 넘는다 */
/* ★★★ 2026-08-17 늦게 (G-14) — **자연광 합격선이 4.80 → 3.68 로 내려갔다.**
   창턱 받침을 방 쪽으로 0.20m 밀어(그루가 창 윗턱을 뚫는 것을 고치려고) 자리가 창
   개구부에서 나왔기 때문이다. 박사님 확정: *"조금만 민다로 하자."*
   ⚠ **아래 세 줄 중 진짜 자물쇠는 `d1 >= 6.0` 이다.** 자연광 값은 「지금 그렇다」이고,
     문턱 6.0 은 「그래야 한다」다(G-16 이 식물등을 창 위로 옮겨 세운 계약). 지금 여유가
     **0.02** 뿐이라, 이 줄이 빨개지면 값을 맞추지 말고 **창턱을 도로 덜 밀어야** 한다.
   ⚠ 3.68 은 몬스테라 정체선(min 3.0) 위다 — 등 없이도 「느림」으로 자란다. 그 아래로
     내려가면 창가 자리가 죽으므로 그것도 같이 못 박는다. */
check('①-b 합격선 — 창턱 자연광 3.68(정체선 3.0 위) · 첫 등이 문턱 6.0 을 넘고 무늬종 8.4 는 안 넘는다', () => {
  unaimed();
  ...
  assert.ok(d1 >= 6.0,
    `창턱이 첫 등으로 갈라짐 문턱을 못 넘습니다(${d1}) — 첫 등값이 화면에서 아무 일도 안 합니다`);
...
/* ⚠⚠ 2026-08-17 (G-14) — **0.5m 가 0.4m 로 좁아졌다.** 창턱 자연광이 4.80 → 3.68 로
   내려가서 등이 메워야 할 몫이 1.20 → 2.32 로 는 탓이다. 실측(같은 식으로 잰 값):
     0.20m 15.34 · 0.25m 11.14 · 0.30m 8.86 · 0.35m 7.49 · **0.40m 6.59** ·
     0.50m **5.54**(못 넘는다) · 1.00m 4.15
   ⇒ 「집게등을 창턱에 물려 살린다」는 길은 **여전히 열려 있다**(창턱 상판에 바로 물리면 훨씬 밝다).
     다만 **여유가 좁아졌다** — 0.5m 로 달던 감각으로는 이제 안 넘는다. 그것도 창턱을
     민 값이다. 표는 docs/handoff/nightstand-to-plan.md. */
check('①-c ★집게등을 창턱 0.4m 안쪽에 달면 문턱 6.0 을 넘는다', () => {
  ...
  /* 자연광에 등 몫을 더한다. 환산비는 이 방의 실측에서 나온다(42.621255 PPFD = 1.84 DLI)
     ⚠ 2026-08-17 — 밑값을 4.80 에서 **3.68**(창턱을 0.20m 민 뒤)로 고쳤다. 밑값을 안 고치면
       이 검사는 **없는 방**을 재게 된다(START-HERE §2.9-⑥: 자가 옛 세상을 재고 있었다). */
  const BASE = 3.68;
  const K = 1.84 / 42.621255;
  assert.ok(BASE + ppfdOf(0.4) * K > 6.0, `0.4m 위에 달아도 문턱을 못 넘습니다`);
  assert.ok(BASE + ppfdOf(1.0) * K < 6.0, `1.0m 위에서도 넘습니다 — 거리가 뜻이 없습니다`);
