...
/* ★★★ 다리를 «한 칸 더» 건넌다 — 「표가 가리킨다」 다음은 「그 사건이 «나기는» 하나」다.
   ⚠ 이게 `monsteraMoved` 부류다: 표에 있고 불리기도 했는데 «조건이 죽어» 있었다.
     ...
     ✔ 그 건은 [core] 가 열었다(2026-08-30 `57d3972`). ★ 그래도 예로 «남긴다» —
       고쳤다고 지우면 다음 사람이 「그런 함정이 있었다」는 것을 모른다.

   ★★ 그리고 «같은 꼴»이 하루에 셋 모였다 (2026-08-30 · [core] 가 셈) —

       monsteraMoved  불리기는 «불렸는데» 조건이 죽어 있었다      ⇒ 검사는 초록
       coachPending   기다리기는 «기다렸는데» 새로고침에 사라졌다  ⇒ 아무도 몰랐다
       numb           뜨기는 «떴는데» «다른 얼굴»이었다           ⇒ 오류 한 줄도 안 났다

     ⇒ ⛔ 셋 다 **「안 죽어서 안 걸린다」**다. **죽었으면 그 자리에서 잡혔다.**
     ⇒ ★ 셋째는 «내가» 낸 것이다 — 파일 이름을 바꾸고 표를 안 고친 사이,
       CSS 가 url 을 둘 주는 덕에 대사가 «neutral 로 조용히» 떴다.
       ⇒ ★★ 그리고 **이 자의 관문이 그것을 잡았다**(「표가 가리키는데 파일이 없다」).
         ⇒ 굴리는 것은 «비싸고 늦다». 관문은 «싸고 그 자리»다.
