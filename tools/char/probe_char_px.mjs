 ...
 *
 * ■ ✔ 그래서 — **게임에게 «묻는다». [core] 가 `worldToScreen` 을 붙여 주었다(9685e8d)**
 *
 *     rv.worldToScreen(x, y, z) → { x, y }   캔버스 기준 px · 읽기만
 *
 * ⇒ ★★ **발밑(y=pos.y)과 머리끝(y=pos.y+키)을 넣으면 «키»가 한 줄에 나온다.**
 *   ⇒ 좌표 변환을 «내가» 안 한다 — 게임의 카메라가 직접 답한다.
 *   ⇒ ⇒ ★ 그리고 픽셀 차이와 달리 **「사람만」이 확실하다.** 몬이도 그림자도 안 섞인다.
 *
