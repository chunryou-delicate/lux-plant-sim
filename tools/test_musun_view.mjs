...
const GOLD = {
  ...
  /* ★★ 2026-08-09 — **몬스테라 그루 높이 여덟 줄이 움직였다.** 회귀가 아니라 확정이다.
     ...
     ★★ 2026-08-17 — **높이 여덟 줄이 또 움직였다. 이번에도 회귀가 아니다.**
     캐논(눈 → 가는 잎자루 먼저 → 그 끝에서 새순)을 지키려고 `plant_grow.html §growTopology`
     에서 **가지 축이 나는 그 날 첫 마디를 같이 심게** 고쳤다. 전에는 마디가 전역 격자에서만
     나서 잎이 잎자루보다 4일 먼저 났다(docs/handoff/monsterabranch-to-plan.md).
     ⇒ `addSeg` 가 난수를 **한 번 더 앞에서** 뽑으므로 마디의 방향·길이가 조금씩 밀린다.
       그래서 **가지가 나기 전(유효 60일)에도** 높이가 움직인다 — 형태의 값이지 시간 축이 아니다.

       유효일   높이 옛 → 새(실측)        화분 0.14 일 때
        0       0.13321 그대로            0.09324 그대로   (씨앗 — 화분만 있다)
     ...
     ⚠ 화분·지름·자식 수는 이번에도 **한 톨도 안 움직였다**(전부 재서 확인).
     ⚠ 여기 적힌 새 값은 **문서에서 옮겨 온 것이 아니라 이 검사로 직접 잰 값**이다
       (START-HERE §2 넷째 규칙 — 고친 값도 재라). */
  monstera: {
