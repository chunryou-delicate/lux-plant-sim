/* ★★ **이 자는 「안내를 따르는 사람」을 잰다 — 표준이다** (2026-08-23 plan)
 *  ------------------------------------------------------------
 *  시루를 **퀴스트대로 늘리고** 돌린다. 처음 하는 사람이 밟는 길이다.
 *  ★ **시루 1개로 버티는 바닥값을 재는 자는 `test_balance_routes.mjs` 다.**
 *    둘이 정반대로 말하면 고장이 아니라 **다른 사람을 재는 것**이다.
 *
