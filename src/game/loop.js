...
function prologueVarieEvents(S, io, stats) {
  ...
  /* ★★ **무늬가 화면에 보이는 날**을 기다린다 (2026-08-13 · 두 번 재서 고쳤다).
    ...
    if (said.length === 1) {
      /* ⚠ 「흰」을 여기 되돌리지 말 것 (2026-08-30 · 박사님 「무늬가 섞였다로 멘트 바꿔」).
         무늬는 흰 것만이 아니다 — 핑크·오로레아·모브·차콜도 무늬다. 값도 흰 것이 제일
         비싸지 않다(`shop.js` 풀문 여섯 중 흰 계열은 하나다). 대사는 이미 「무늬」인데
         이 기록만 「흰」이면 한 화면에서 둘이 어긋난다. */
      pushLog(S, '✨ 새로 난 잎에 무늬가 섞였습니다 — ' +
...
export function harvestCrop(S, io, opt = {}) {
  ...
  try {
    ...
    /* ★★ 2026-08-15 — **「못 받았습니다」로 끝나면 손해만 남는다.**
       ------------------------------------------------------------
       이 줄은 손해와 처방(날을 달리해라)까지만 말하고 끝났다. 그런데 2026-08-15 밤에
       **못 받은 몫을 파는 자리가 생겼다**(상점 [잉여 채소 넘기기] · pantrysale-to-plan).
       그 자리가 생긴 것을 아는 화면이 [상점] 탭 하나뿐이라, 겹쳐 거둔 사람은 자기가
       손해 본 줄만 알고 **되찾을 길이 있다는 것을 모른 채** 지나갔다(guideaudit §1-8 ①).
       ⚠ **얼마에 넘어가는지는 여기 안 적는다.** 값(85%)은 `cropSurplusQuote` 가 정본이고
         상점이 그때그때 읽어 말한다 — 여기에 베끼면 낡은 숫자가 하나 더 생긴다. */
    const sellTail = ' 못 받은 몫은 **[상점]에서 넘길 수 있습니다**';
    /* ⚠⚠⚠ 2026-08-17 — **이 줄도 닿을 수 없는 자리가 됐다.**
       ------------------------------------------------------------
       박사님이 겹침의 벌을 걷으셨다(*"하루 수확량을 개수에 따라 조절하라는 게 아니었는데…
       식량으로 사용할 수 있는 G수를 조절하란 거지"*  · first_play §겹침 2026-08-17).
       ⇒ `r.overlapLostWon` 은 이제 **늘 0** 이라 이 줄은 안 뜬다.
       ★ 바로 아래 「곳간이 넘쳐 쉬었습니다」와 **같은 처지·같은 처분**이다 — 지우지 않는다.
         `rules.cropOverlapTiredEnabled` 문이 열리면 그대로 살아나고(§겹침), 그때 이 문구가
         말하는 것(「덜 챙겼다 · 날을 달리해라」)이 다시 참이 된다.
       ⚠ 대신 **화면이 조용해진 것은 사실이다.** 이제 같은 날 몰아 거둬도 화면이 아무 말도
         안 한다 — 실제로 손해가 없기 때문이다. 남은 손해는 「하루 300g 을 넘겨 판 만큼
         15% 를 덜 받는다」 하나이고, 그건 밥상 창과 상점이 말한다.
       ⚠ 2026-08-16 — 그 전에도 한 번 고쳤다: 「곳간이 안 비어 있어」는 곳간 한도가 없어지면서
         이미 틀린 까닭이 되어 **질림**으로 바꿔 놓았다. 그 뜻도 이번에 같이 잠긴다. */
    ...
    if (!fp.monstera.arrived && monsteraArrivalDue(fp)) {
      ...
      /* ══ ★★★ 2026-08-23 — **자리를 다 채우면 판이 잠기던 것** (하네스가 잡았다) ═══════
         ------------------------------------------------------------
         옛 줄: `if (!spot) throw …` — `spot` 은 **작물 자리를 뺀 뒤**의 후보다.
         그래서 퀘스트가 시키는 대로 시루를 늘려 **자리를 다 채운 판**에서, 첫 수확 때
         이 줄이 던졌다. `harvestCrop` 이 화면의 `guard()` 안에서 도는지라
         **판이 통째로 잠겼다**(`hardLock=1` · [다음 날]이 영영 안 눌린다).
         ⇒ 실측: `night_play --play guided` 가 **Day 8 에 그렇게 죽었다.**
           안내를 무시한 판(시루 하나)은 안 죽는다 — **시키는 대로 한 사람만 죽는다.**

         ★ 문지기가 **두 가지를 못 가르고 있었다:**
           ㉠ 이 방에 화분이 올라갈 자리가 **원래 없다**   → 방 데이터 문제. 던지는 게 맞다
           ㉡ 지금 작물이 **다 차지하고 있다**             → 판의 형편. 던지면 안 된다
         ⇒ ㉠ 은 바로 위 `canHoldPot.size` 가 이미 본다. 여기서는 ㉡ 만 걸리고 있었다.
         ⇒ ★ 그리고 **화분은 어차피 `slotId: null` 로 가방에 온다**(2026-08-17 §선물은 가방으로).
           자리가 필요 없어졌는데 자리를 요구하는 문지기만 남아 있던 것이다.

         ⇒ 그래서 **작물을 뺀 후보가 비어도 안 던진다.** 대신 「이 방에 놓을 수 있는 칸이
           빛 보고까지 서 있나」만 본다 — 그것이 ㉠ 이다.
         ⚠ `spot` 셈은 그대로 둔다. 지우면 「어느 자리가 좋은가」를 다시 셀 때 근거가 사라진다. */
      const roomHasSpot = (report.slots || []).some(s => s && canHoldPot.has(s.slotId) &&
                            typeof s.dli === 'number' && isFinite(s.dli));
      if (!roomHasSpot)
        throw new Error('[첫 플레이] 몬스테라가 도착할 화분 자리를 찾지 못했습니다 — ' +
                        '이 방에는 화분이 올라갈 칸이 없습니다(방 데이터를 확인해 주세요)');

      /* ★★★ 2026-08-17 — **선물은 가방으로 온다** (박사님: *"처음에 몬스테라 주는 거
           인벤에 줘서 드래그해서 배치하거나 누르면 좌측이나 우측 바닥에 배치되도록 해
           (기존 배치된 거 피해서)"*)
         ══════════════════════════════════════════════════════════════
         ⚠ 예전에는 **책상 칸에 저절로 놓였다.** 그러면 「어디에 둘까」라는 이 게임의 첫
           물음이 통째로 건너뛰어지고, 플레이어는 자기가 안 고른 자리에서 시작한다.
           (그 자리를 고르느라 위 `desks[0] || 제일 어두운 칸` 이라는 셈까지 있었다.)
         ⇒ **자리를 안 준다.** `slotId: null` 이면 화분은 「아직 아무 데도 안 놓은」 상태로
           남고(§syncRoom 이 그 상태를 이미 안다 — 방에 부탁하지 않는다), 가방 칸으로 서서
           끌거나 눌러서 놓는다. 누르면 바닥 구석에 선다(§provisionalSpot).
         ⚠ 위 `spot` 셈은 **안 지웠다** — 못 놓을 자리뿐인 방을 가려내는 문지기로 아직 쓴다.
           자리가 하나도 없으면 여기서 던지는 것이 맞다(방이 잘못된 것이다). */
      arrived = givePlant(S, io, { slotId: null });
