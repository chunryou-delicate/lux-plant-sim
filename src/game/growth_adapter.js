...
/* ⚠★ 2026-08-29 — **이 값을 바꾸면 `src/game/dialogue.js` 의 `monsteraArrived` 도 같이 바꿔라.**
     몬이가 거기서 「**두 번째랑 세 번째** 잎에 무늬가 나」라고 **글자로** 말한다.
     ⚠⚠ 2026-08-30 — **이 줄이 하루 만에 낡았다.** 어제 여기 「흰 무늬」라고 적었는데
       그날 저녁 박사님이 대사에서 「흰」을 빼셨다. 가리키는 줄이 옛말을 이고 있으면
       다음 사람이 그것을 믿고 되돌린다 — **낡지 말라고 붙인 줄이 제일 먼저 낡았다.**
     ⇒ 그러니 이 줄도 **베끼지 말고 가리키기만 한다.** 대사의 낱말을 여기 다시 적으면
       그 사본이 또 낡는다. 무엇을 말하는지는 저쪽에서 읽어라([Plan] 2026-08-30).
     ★ 글자로 박은 까닭은 대사라서다 — 「둘째·셋째」를 변수로 읽으면 말이 안 된다.
     ⇒ ★★ 그 대신 **가리키는 줄을 붙여 같이 낙게 한다**(㉶ · [Plan] 2026-08-29). */
