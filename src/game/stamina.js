...
export const STAMINA_RULES = Object.freeze({
  ...
  /* ══ 퀘스트가 주는 최대체력 ═══════════════════════════════════════════
     ...
     ★★ 2026-08-16 — **여섯 줄이 됐는데 체력을 주는 것은 여전히 셋뿐이다**(5 → 8).
       나머지 셋은 **0 이고 그게 뜻이다**:
         `buy_lamp`     보상은 **밝은 자리가 생기는 것**이다 (2026-08-16 확정 — 등이 곧 길)
  ...
  quests: Object.freeze({
    /* ══ ★★★ 초반 사슬 여덟 줄 (2026-08-16 · quest.js §초반 사슬) ══════════════
       박사님 확정: **「조금 준다 — 굵은 마디 셋만 +1」.**
       ⚠ 여덟 줄에 다 주면 첫 플레이 최대 체력이 **5 → 13** 이 된다. 그러면 손이 두 배 반이라
         시루를 몇 개든 돌릴 수 있게 되고, **퀘스트 ②가 가르치는 「체력이 천장이다」가
         첫 플레이에서 통째로 사라진다.** 하나도 안 주면 초반이 숨이 트이지 않는다.
       ⇒ 가운데를 잡았다 — **5 → 8.** 「굵은 마디」란 그 걸음을 지나면 판이 넓어지는 자리다:
         첫 수확(회전이 돈다) · 시루 늘리기(하루가 바빠진다) · 몬스테라 자리(빛이 붙는다).
       ⚠ 잔 걸음(놓기·물주기·다시 심기·잎 2·3장)은 **0 이다.** 그 줄들의 보상은
         「세상이 열리는 것」이고, 그것이 이미 보상이다(`quest.reward` 가 그렇게 말한다). */
    place_siru:    0,
    water_siru:    0,
    first_harvest: 1,   // ★ 회전이 돌기 시작한다
    resow_siru:    0,
    siru_two:      1,   // ★ 하루가 바빠진다 — 그 순간 손이 하나 더 필요하다
    monstera_home: 1,   // ★ 빛이 붙는다 — 물주기가 하루에 하나 더 늘어난다
    leaf_two:      0,
    leaf_three:    0,
    siru5_cycle5: 1,   // 시루 다섯, 다섯 바퀴 — 「물은 한 번에 하나 · 체력이 천장」
    crop_mix:     1,   // 한 상에 두 가지 — 「섞어 먹어야 밥이 이득」
    /* ★ 2026-08-16 — 시루 늘리기 둘. 박사님: *"퀘스트로 (…) 개수 달성하도록 (체력도 1 주고)"*
       ⚠ 이 둘은 **체력이 곧 보상**이다 — 체력이 늘어야 시루를 더 굴릴 수 있고(하루 N번),
         그래야 다음 문턱에 닿는다. 보상과 목표가 같은 축인 유일한 줄이다. */
    siru8:        1,   // 여덟까지 늘린다
    siru16:       1,   // 열여섯이면 버틴다 (본전)
    first_cut:    1,   // 물에 꽂아 본다 — 「잎 1장까지 쪼개야 물꽂이」
    buy_lamp:     0,   // ★ 보상은 밝은 자리가 생기는 것이다
