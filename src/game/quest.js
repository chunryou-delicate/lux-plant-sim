...
const FIRST_PLAY_CHAIN = Object.freeze([
  ...
  Object.freeze({
    ...
    ko: '물을 준다',
    speaks: true,
    /* ★ 2026-09-25 [plan] #2 — 「회전」은 아직 안 배운 낱말이다(첫 수확 뒤에 배운다). 보이는 말로 */
    reward: '콩나물이 자라기 시작합니다',
  ...
  Object.freeze({
    ...
    /* ★ 2026-09-25 [plan] #2 — 이 줄은 물 준 그 순간(D0)에 열린다. 그날부터 D4 까지 「다 자란 콩나물을
       거두세요」는 **없는 것을 시켰다**(거둘 게 없다). 익은 용기가 있을 때만 거두라 하고, 아니면 날을 넘기라 한다.
       ⚠ `ready` 는 화면(questSnapshotNow)이 코어 `cropPotList` 의 그 칸을 옮겨 준다 — 여기서 날짜를 다시 안 센다.
       ⚠ 스냅샷이 없으면(검사가 `questTodo(q)` 로 부를 때) «자라는 중» 갈래다 — 그 줄도 28자 안이다. */
    todo: (d, s) => (arr(s && s.cropPots).some(p => yes(p && p.ready))
      ? '다 자란 콩나물을 거두세요'
      : '콩나물이 자라는 중 — [다음 날]로 넘기세요'),
  ...
  Object.freeze({
    ...
    /* ★ 2026-09-25 [plan] #6 — 그루는 이제 가방으로 온다(「온 자리」가 없다). 「7일 평균」·「나흘」은 엔진의 수라 안 읊는다 */
    why: '가방 안이나 어두운 자리에서는 자라지 않습니다. 밝은 자리에 두면 며칠 뒤부터 달라집니다.',
...
const MAIN_QUESTS = Object.freeze([
  ...
  Object.freeze({
    ...
    /* ★ 2026-09-06 아침 — 박사님이 걸어 보시고 직접 내신 문안([plan] 정본): 어디서·어떻게가 있고 수는 칸(need.sirus)에서 */
    todo: q => `상점에서 시루를 더 주문해 ${q.need.sirus}개 이상 배치하세요`,   /* [plan] ⓑ — 박사님 낱말(상점·더·주문·N개 이상·배치) 살리고 28자 계약 안(26자) */
  ...
  Object.freeze({
    id: 'buy_lamp',
    /* ★ 2026-09-25 [plan] #3 — 첫 등은 공짜로 가방에 온다(game.html L.owned=1). 「사 온다」가 아니라 「들인다」.
       ⚠ id·열쇠·done·보상은 그대로다 — 이름과 할 일 글자만 바꿨다. */
    ko: '볕을 들인다',
    /* ★ 체력이 0 인 까닭이 곧 이 이름이다 — 보상은 **밝은 자리 그 자체**다 */
    reward: '밝은 자리가 생깁니다',
    teaches: ['등이 곧 밝은 자리다', '반지하는 등 없이 못 나간다'],
    why: '반지하에는 밝은 칸이 거의 없습니다. 등을 켜야 그 자리가 생깁니다.',
    todo: () => '식물등을 달아 밝은 자리를 만드세요',
