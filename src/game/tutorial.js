...
export const TUTORIAL_RULES = Object.freeze({
  ...
  /* ★★ 2026-08-09 박사님 확정(엑셀 직접 수정) — **1,300,000 → 1,500,000원.**
     월세를 20만으로 내린 것(§rentWon)과 **한 벌**이다. 둘이 같이 움직여
     「작물 없이 두 달」이 「작물 없이 석 달」이 된다 — 박사님 대사가 그제야 맞는다. */
  startCashWon: 1_500_000,
  /* ★★ 2026-08-09 박사님 확정 — **1,500,000 → 2,000,000원.**
     ------------------------------------------------------------
     ⚠ **시작돈을 150만으로 올린 그 순간 이 값이 시작돈과 같아졌다.** 그러면 돈 조건이
       **첫날부터 참**이라 배움 넷만 채우면 나간다 — 실측으로 튜토가 게임 48일에 끝나고
       가을·식물등·겨울이 통째로 건너뛰어졌다. 목표 줄이 「이사 자금 N원 남았습니다」를
       **한 번도 안 말했다.** 시작돈과 이사비는 **같이 움직여야 하는 한 벌**이다.

     ★ 실비 근거를 다시 썼다(옛 주석은 「원룸 보증금 + 첫 달 월세 + 이사비」였다):
         원룸 보증금        1,000,000
         첫 달 월세 선납      350,000   (원룸 월세 35만 — 반지하 20만보다 비싸다)
         이사비·중개·잡비     650,000
       박사님 원문: *"200 가지고 가서 보증 100 내고 첫달 35 선납하고선 65만원 시작하는 거지."*
       ⇒ **남는 65만원이 원룸에서의 초기 자금**이다. 그게 이 값의 뜻이다.

     ★ 왜 하필 200만인가 — **무늬 등급 계단**이 그 자리에 있다. 잎 11장 중 무늬 3장이면
       하프문(61배) 191만원이라 150만은 **41만이 남아돌아 마지막이 싱겁다.** 200만이면
       하프문 하나로 9만원이 모자라, **삽수를 더 팔지 잎을 하나 더 기다릴지**를 한 번 고르게 된다. */
  moveOutCostWon: 2_000_000,
  ...
  /* ★★ 2026-08-09 박사님 확정(엑셀 직접 수정) — **300,000 → 200,000원.**
     정본은 `data/balance/homes.json` 의 `banjiha.rent` 이고 여기 값은 그 파일을 못 읽는 판을
     위한 **폴백**이다(§banjihaRulesFrom 이 읽어 꽂는다). 둘이 같은지는 `tools/test_econ.mjs` 가 본다.
     ⚠ 아래 `dailySpendWon` 이 **따라와야 한다** — 월세만 내리면 하루치에서 빼는 몫도 같이
       줄어 총액이 그대로다(2026-08-05 에 실제로 그랬다: 월세만 내렸더니 파산일이 81일 그대로). */
  rentWon: 200_000,               // 반지하 월세 — data/balance/homes.json §banjiha.rent
  ...
  /* ★★ 2026-08-09 — 월세가 20만으로 내려갔으므로 여기도 따라 내린다(위 §rentWon).
       월세 6,667 + 공과·전기 2,500 + 식비 7,500 = **16,667원/일** = 월 50만.
       셋 다 `data/balance/homes.json`(rent 200,000 · utility 75,000)과
       `data/balance/characters.json._meta`(dailyFoodPerPerson 7,500)에서 나온 값이고,
       `banjihaRulesFrom` 이 그 식(월세/주기 + 공과/주기 + 식비)으로 **다시 셈해** 덮는다 —
       여기 숫자는 그 셈과 같아야 하는 폴백이다(`tools/test_econ.mjs` §A 가 등식을 고정한다).
     ⚠ 이 값과 `rentWon` 은 **같이 움직인다.** 한쪽만 바꾸면 총액이 안 바뀌거나 두 번 샌다
       (2026-08-05 에 실제로 그랬다 — 월세만 내렸더니 파산일이 81일 그대로였다). */
  dailySpendWon: 16_667,
...
export function tutorialDay(ts, opt = {}) {
  ...
  /* ⚠⚠⚠ 2026-08-27 — **여기 있던 인용이 «죽었다».**
     있던 말: *"★안 사는 것도 답이다 (story_arc.md §4) — 그래서 재촉이 아니라 한 번만 짚는다."*
     ⇒ ⛔ `story_arc.md` §2 경로 A(등 없이 나가는 길)가 **2026-08-24 에 폐기됐다**
       (박사님 2026-08-16 「등 없이 나가는 길은 안 만든다」 · [Plan] `a7bfe97` 이 문서에 정정을 박았다).
     ⇒ ★ 그러면 「안 사는 것도 답이다」가 **더 이상 근거가 없다.** 등은 «선택»이 아니라 «길»이다.

     ⚠⚠ **그래도 이 줄의 «동작»은 안 바꾼다.** 바꾸면 「한 번만 짚기」가 「재촉」이 되는데,
       그것은 **판을 가르는 결정**이라 이 창이 정할 것이 아니다(박사님·[Plan] 몫).
     ⇒ ★ 여기서는 **낡은 인용만 걷는다.** 「무엇으로 바꿀까」는 위 둘이 정한다.
     ⇒ ⚠ 그리고 같은 뿌리에서 온 대사가 하나 더 있다 — `dialogue.js` 의
       「사도 되고 «안 사도» 돼」. **그 줄도 같은 답을 기다린다.**
