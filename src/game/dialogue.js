...
export const SCRIPTS = {
  ...
  monsteraArrived: [
    ...
    /* ★★ 2026-08-29 [Plan] — **「잘 키우면」을 「정해져 있다」로.**
       ⚠ 바로 윗줄은 «자리» 이야기(「어두운 데 두면 아무 일도 안 일어나」)인데
         이 줄은 «노력» 이야기였다 — 두 줄 만에 몬이가 다른 말을 했다.
       ⛔ 그리고 「잘 키우면」은 **틀렸다** — `PROLOGUE_VARIE_LEAVES = [2,3]` 이라
         둘째·셋째 잎은 «정해져» 있다. 「잘 키우면」은 «내기»의 말인데 실제는 «약속»이다.
  ...
  /* 식물등 해금 — ★필수품이 아니라 **선택**이다(story_arc.md §4).
     그래서 몬이가 "사"라고 하지 않는다. 판단은 넘긴다.
     ⚠ (09-25 바뀜) 예전엔 값·기간을 대사로 줬다 — 지금은 전기값 한 줄만 준다. 값 줄을 되살리지 말 것(아래 ★★★ #3). */
...
```
  ...
  /* ★★★ 2026-08-30 [Plan]·박사님 — **「흰 게」를 「무늬가」로.**
     ⛔ 「흰 게」는 **거짓말이 되는 판이 있다** — 갤럭시-다크틸·숯빛이 나온 사람에게는
       흰 데가 **없다.** 「무늬가 섞였어」는 **어느 그림이든 참**이다.
     ★ 그리고 그 한 줄이 「그림이 3px 에서 안 보이는 것」까지 참으로 만든다 —
       「무늬가 섞였어」는 **그림을 안 봐도** 참이다.
     ⚠ 아랫줄 「무늬 진 데는 빛을 못 만들어서」가 **더 크다** — 「났다」가 아니라 **「왜 나는가」**를
...
export const CHATTER = [
  ...
  /* ★ 2026-08-27 — 두 번째로 나오는 날의 한 줄([Plan] · §chatMoniName2).
     ...
     ⇒ ⛔ **그리고 «안 만들기로» 했다** ([Plan] 2026-08-27). 까닭이 이 줄 «밖»에 있다:
```
       잡담 «일곱»이 다 되풀이된다 — 하루지출 · 아침 · 조용함 · 몬이이름 · 부모 · 집주인 · 윗집
       ⇒ ★ 이 하나에만 장치를 달면 «그것만 튄다». 나머지 여섯은 그대로 되풀이되니까
       ⇒ ⇒ 일곱을 «한 벌»로 볼 때 같이 정한다. 그건 새 일이고 박사님 몫이다
```
     ⚠ 그리고 「어색하다」는 **아직 안 재어진 것**이다 — 「…아직도 몬이네」는 되풀이돼도 «참»이고,
       사람은 같은 것을 또 묻기도 한다. **박사님이 보시다가 걸리면 그때가 답이다.** */
