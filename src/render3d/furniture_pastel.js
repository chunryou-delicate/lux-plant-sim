...
function tierSlots(w, y, n, z=0, margin=null){
  const out=[];
  const A = topAxis(w);
  /* ★★ 2026-08-23 — **자리 하나짜리가 「칸 경계」에 앉아 영영 못 쓰는 자리가 되어 있었다.**
     ------------------------------------------------------------
     예전 주석은 이랬다: *"n===1 은 예전대로 한가운데다. … 짝수면 칸 경계인데 —
     걸음(0.125)이 칸의 절반이라 **경계도 앉을 수 있는 자리다.**"*
     그 말은 **옛 정본**에서만 참이었다. CORE 가 뒤집었다 —
       `game/room_view.js` §snapAxis: *"전에는 「0.125 격자 위의 점」이 정본이고 칸이 그걸
        따라 그려졌다. 이제 **칸이 정본이고 놓는 자리가 칸 한가운데**다."*
     커서는 이제 **칸 한가운데에만** 내려앉는다. 곧 경계는 겨눌 수 없는 점이다.
     그런데 여기는 옛 정본대로 상판 한가운데를 냈다. 짝수 칸 상판에서는 그것이 곧 경계다.

     터진 자리 — 협탁이 `ca3f8f8` 로 0.42×0.36 → 0.50×0.50(= **2×2 칸**)이 되면서
     상판 한가운데가 칸 교차점이 되었다. 가장 가까운 칸 한가운데까지 **0.1768m**,
     `SLOT_GOVERN_R` 0.04 의 네 배가 넘는다. ⇒ **어느 칸을 겨눠도 이 자리로 안 붙는다.**
     추천 자리가 그려져 있는데 못 쓴다. 반지하·원룸·투룸·아파트 네 방이 그랬다.

     ⇒ 자리 하나도 **칸 한가운데**에 앉힌다. 홀수 칸이면 결과가 예전과 같고(창턱 받침
       0.36m = 1칸이 그렇다 — 재서 확인했다), 짝수 칸이면 반 칸 옮겨 붙는 자리가 된다.
     ⚠ 짝수일 때 양쪽 칸이 **똑같이 가깝다.** 여기서는 앞 칸(작은 번호)으로 정한다 —
       한쪽으로 정해 두지 않으면 빌더마다 달라진다. 어느 쪽이든 밝기는 거의 같다
       (협탁 실측 x −0.125 / +0.125 에서 peak 0.44 / 0.43).
     ★ 여기서 **자리 수는 안 늘린다.** 2×2 상판에 네 칸을 주면 더 좋다는 것은 재 뒀지만
       (peak 0.44·0.43·0.32·0.32, 넷 다 닿는다) 그건 **자리를 늘리는 일**이고 자리 수는
       이 게임의 밸런스다(시루 16개가 이사 문턱). 고장 고치는 일과 섞지 않는다.
       근거는 `docs/engine/slot_cell_align.md` 에 남겼다 — 사용자 결정 대기. */
  if(margin==null && n===1){
    let k=0, bd=Infinity;
    for(let i=0;i<A.n;i++){ const d=Math.abs(A.at(i)); if(d<bd-1e-9){ bd=d; k=i; } }
    return [{ x:+A.at(k).toFixed(3), y:+y.toFixed(3), z }];
  }
...
B.shelf=(o)=>{
  ...
  g.userData.size={w,h,d};
  const tiers=[], sl=[];
  /* ★★ 2026-08-23 — **단마다 상판 폭이 다른데 한 폭으로 자리를 냈다.**
     i=0 은 **밑판**(`bx(w,t,d,…)` — 통짜, 폭 w)이고 i≥1 은 **선반**(`bx(w-2t,…)` —
     측판 사이, 폭 w−2t)이다. 화면은 광선이 **맞은 판때기**로 칸을 그리므로
     (`room_view.tierRectOf` → `meshRect`) 밑단은 1.00 로, 위 단은 0.93 으로 나뉜다.
     그런데 여기서는 둘 다 0.93 으로 자리를 냈다 ⇒ **밑단만 칸에서 어긋났다**
     (낮은 책장 1.00m: 0.065 · 0.125 · 0.065m — 재서 확인). 단마다 제 폭을 준다.
     ⚠ 자리 **수**는 그대로다(`w>0.9?3:2`) — 폭만 맞춘다. */
  for(let i=0;i<n;i++){ const y=h*i/n+t/2; tiers.push(+y.toFixed(3));
    sl.push(...tierSlots(i===0 ? w : w-2*t, y, w>0.9?3:2)); }
...
B.nightstand=(o)=>{
  ...
  g.userData.size={w,h,d};
  /* ★ 깊이축도 칸 한가운데로 — 0.50 깊이는 2칸이라 한가운데가 경계였다.
     `frontCellZ` 는 책상·서랍장이 이미 쓰는 자다(방 쪽 앞줄). 협탁은 벽에 붙으므로
     앞줄이 손이 닿는 쪽이고, 재 보니 밝기도 그쪽이 낫다(0.44 vs 0.32). */
  return addSlots(g, tierSlots(w, h, 1, frontCellZ(d)), [h]);
};
...
/* 천장등 — **천장에 붙는 LED 판** (ceilY 아래로 매달림)
   ...
   ★★ 2026-08-23 박사님: ***"그대로. 떠 보이는 것으로."***
     ⚠⚠ **이 판은 「떠 보인다」. 그런데 값은 맞다** — y 2.250~2.300, 천장이 2.3 이다. **딱 붙어 있다.**
       떠 보이는 까닭은 **이 방이 「천장이 안 그려지는 각도」**라서다. 붙을 데가 화면에 없다.
       ⇒ 예전에는 **줄**이 그 일을 했다 — 줄이 위로 뻗어 「저 위 어딘가에 매달렸다」로 읽혔다.
         줄을 없애니 **지지가 사라졌다.**
     ⇒ 고치는 안이 셋 있었다(짧은 목 2~3cm · 더 얇고 크게 · 등 위에 작은 천장 판).
       **박사님이 「그대로」를 고르셨다.** 까닭은 [Asset] 이 갈라 적은 그것이다 —
       *"요즘 LED 판은 천장에 완전히 밀착한다. **그림에서 안 읽히는 것이 문제이지
         실물이 틀린 게 아니다.**"*
     ⛔ **그러니 「떠 있는 버그」로 보고 고치지 마라.** 2026-08-23 에 [Asset]·[House]·[Master]
       셋이 각각 찾아냈고, **셋 다 같은 결론에 닿았고, 안 고치기로 정해졌다.** */
B.lamp_ceiling=(o)=>{
